...
class EnrichPharmacyFaxCommand extends Command
{
    /**
     * The name and signature of the console command.
     */
    protected $signature = 'pharmacies:enrich-fax
                            {--city= : Filter pharmacies by city}
                            {--state= : Filter pharmacies by 2-letter state code}
                            {--limit= : Maximum number of pharmacies to process}
     ...
     */
    protected $description = 'Enrich existing pharmacies with verified NPI and Fax numbers from CMS NPPES Registry';

    /**
     * Execute the console command.
     */
    public function handle(PharmacyFaxEnrichmentService $service): int
    {
        ...
        $this->newLine();
        $this->line('============================================================');
        $this->info('      Pharmacy Fax & NPI Enrichment' . ($dryRun ? ' — DRY RUN' : ''));
        $this->line('============================================================');
        ...
        if (empty($results)) {
            $this->warn('No pharmacy records found matching the specified criteria.');
            return 0;
        }
        ...
        foreach ($results as $index => $r) {
            $num = $index + 1;
            $this->newLine();
            $this->line("<comment>[#{$num}] Local Pharmacy:</comment> {$r['entity_name']}");
        ...
        $this->table(
            ['Metric', 'Count'],
            [
                ['Total Pharmacies Checked', $stats['checked']],
